...
import { BookOpen, Clock, FileText, Bell, LayoutDashboard, ChevronRight, Users } from 'lucide-react';
...
function CoursesContent() {
  ...
  const menuItems = [
    ...
    { label: 'My Courses', icon: BookOpen, href: '/dashboard/teacher/courses', active: true },
  ...
  return (
    <DashboardLayout menuItems={menuItems} title="My Courses" subtitle={`${courses.length} courses`} role="Teacher">
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {courses.map((course: any) => (
            <div key={course.id} onClick={() => router.push(`/dashboard/teacher/courses/${course.id}`)}
